...
def Inverter_pow_loss(Vp,Rq_Inv,Ip,fi,Vq_th_v,Rd_Inv,Vd_th_v,Vbat):
    mi=2*Vp/Vbat
    Pq_Inv=(1/8+mi/(3*math.pi))*Rq_Inv*Ip**2+(1/(2*math.pi)+mi/8*math.cos((fi))*Vq_th_v)
    Pd_Inv=(1/8-mi/(3*math.pi))*Rd_Inv*Ip**2+(1/(2*math.pi)-mi/8*math.cos(fi)*Vd_th_v)
...
def electromechenical_torque_ps(js,ws,dt,Bv,Tc,Ts):
    Te=js*ws/dt+Bv*ws+Tc+Ts
    ps=ws*Ts
    return Te,ps


def electric_machine(Rs1, Ld1, Lq1, lamda_pm1,id1, iq1, t1, T_c1,s, Bv1, Js1, P1):
    # Define symbols
    ...
    vd = sp.Eq(vd, Rs * i_d + Ld * di_d_dt - we * Lq * i_q)
    vq = sp.Eq(vq, Rs * i_q + Lq * di_q_dt + we * Ld * i_d + we * lamda_pm)
    ...
    vd_eq_substituted = vd.subs(substitutions)
    vq_eq_substituted = vq.subs(substitutions)
    Te_eq_substituted = Te_eq.subs(substitutions)
    ps_eq_substituted = ps_eq.subs(substitutions)

    # Return substituted equations
    return vd_eq_substituted, vq_eq_substituted, Te_eq_substituted, ps_eq_substituted,Pem_eq
